...
output_dir = "/home/acances/Code/TVHID/interaction_criterion_evaluation"
...
def is_segment_considered_positive(segment, track_1, track_2, IOU_THRESHOLD, FRAME_PROPORTION):
    begin, end = segment
...
def is_segment_considered_negative(segment, track_1, track_2, IOU_THRESHOLD, FRAME_PROPORTION):
    begin, end = segment
...
def get_statistics(video_id, IOU_THRESHOLD, FRAME_PROPORTION):
    ...
    for id_1, id_2 in segments:
        ...
        for segment in segments[(id_1, id_2)]:
            ...
            considered_positive = is_segment_considered_positive(segment, track_1, track_2, IOU_THRESHOLD, FRAME_PROPORTION)
            considered_negative = is_segment_considered_negative(segment, track_1, track_2, IOU_THRESHOLD, FRAME_PROPORTION)
...
def compute_statistics(IOU_THRESHOLD, FRAME_PROPORTION, silent=True):
    # TP, FP, TN, FN, IP, IN
    labels = ["handShake", "highFive", "hug","kiss", "negative"]
    statistics = [0 for i in range(6)]
    statistics_by_label = {}
    for label in labels:
        statistics_by_label[label] = [0 for i in range(6)]

    video_ids = [e.split("/")[-1].split(".")[0] for e in glob.glob("{}/*".format(GT_dir))]
    for video_id in video_ids:
        video_stats = get_statistics(video_id, IOU_THRESHOLD, FRAME_PROPORTION)
        label = video_id.split("_")[0]
        for i in range(6):
            statistics[i] += video_stats[i]
            statistics_by_label[label][i] += video_stats[i]
    ...
    if not silent:
        ...
        for label in labels:
            TP, FP, TN, FN, IP, IN = statistics_by_label[label]
            print(label)
            print("TP: \t {} \t FP: \t {} \t IP: \t {}".format(TP, FP, IP))
            print("TN: \t {} \t FN: \t {} \t IN: \t {}".format(TN, FN, IN))

    with open("{}/stats_IoU{}_prop{}.csv".format(output_dir, IOU_THRESHOLD, FRAME_PROPORTION), "w") as f:
        f.write(",".join(map(str, statistics)) + "\n")
        for label in labels:
            f.write(",".join(map(str, statistics_by_label[label])) + "\n")

    total = sum(statistics)
    percentages = [round(e/total*100, 1) for e in statistics]
    percentages_by_label = {}
    for label in labels:
        total = sum(statistics_by_label[label])
        percentages_by_label[label] = [round(e/total*100, 1) for e in statistics_by_label[label]]

    with open("{}/stats_IoU{}_prop{}.csv".format(output_dir, IOU_THRESHOLD, FRAME_PROPORTION), "a") as f:
        f.write(",".join(map(str, percentages)) + "\n")
        for label in labels:
            f.write(",".join(map(str, percentages_by_label[label])) + "\n")

if __name__ == "__main__":
    for IOU_THRESHOLD in [0.1, 0.2, 0.3, 0.4]:
        for FRAME_PROPORTION in [0.1, 0.2, 0.3, 0.5, 1.0]:
            compute_statistics(IOU_THRESHOLD, FRAME_PROPORTION)
    gather_results()
